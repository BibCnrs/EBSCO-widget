...
describe('articleSearch', () => {
    ...
    it('should translate ArticleSearch', () => {
        ...
        cy.get('.navbar.navbar-default', {timeout: 1000}).contains('Revue, ouvrage');
        cy.get('.navbar.navbar-default .language', {timeout: 1000}).contains('fr');
        cy.get('.search-input .term input', {timeout: 1000}).should('have.attr', 'placeholder', 'Rechercher des articles, des chapitres de livre, des DOIs, des auteurs, des mots du résumé du titre, ISSN, ISBN.');
        ...
        cy.get('.navbar.navbar-default', {timeout: 1000}).contains('Article');
        cy.get('.navbar.navbar-default', {timeout: 1000}).contains('Journal, book');
        cy.get('.search-input .term input', {timeout: 1000}).should('have.attr', 'placeholder', 'Search articles, book chapters, DOIs, authors, words from the title abstract, ISSN, ISBN.');
