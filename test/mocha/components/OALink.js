...
describe('OALink', function() {
    ...
    it('should proxify doaj links', function() {
        ...
        assert.deepEqual(
            a.props().href,
            'http://api.local/oa?url=https://doaj.org/article/579ba7d670e24071a9ac4c06de8b1b7f&sid=oa&domaine=TEST&doi=TEST-01',
        );
    ...
    it('should proxify arxiv links', function() {
        ...
        assert.deepEqual(
            a.props().href,
            'http://api.local/oa?url=http://arxiv.org/abs/1301.1629f&sid=oa&domaine=TEST&doi=TEST-01',
        );
        assert.deepEqual(a.props().children, props.children);
    });
    ...
    it('should proxify hal links #1', function() {
        ...
        assert.deepEqual(
            a.props().href,
            'http://api.local/oa?url=https://www.hal.inserm.fr/inserm-01802849&sid=oa&domaine=TEST&doi=TEST-01',
        );
        assert.deepEqual(a.props().children, props.children);
    });
    ...
    it('should proxify hal links #2', function() {
        ...
        assert.deepEqual(
            a.props().href,
            'http://api.local/oa?url=https://hal.inserm.fr/inserm-01802849&sid=oa&domaine=TEST&doi=TEST-01',
        );
        assert.deepEqual(a.props().children, props.children);
    });
});
