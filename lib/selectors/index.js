...
export const hasMetadoreSearchResult = createSelector(
    [getSearchResult],
    searchResult =>
        searchResult.metadore && searchResult.metadore.totalHits > 0,
);
export const {
    getCurrentPage,
    getCurrentPageData,
...
// si page = article ou si recherche effectué sur un domaine non existant => false
...
export const getMetadoreQueryString = createSelector(
    [
        getCurrentQueryList,
        getSearchResultsPerPage,
        getCurrentPage,
    ...
    (queries, resultsPerPage, currentPage, nextLink) => {
        if (currentPage > 1) {
            return `${nextLink}&currentPage=${currentPage}`.substring(1);
        }
        return buildQueryString({
            metadoreQuery: queries[0].term,
            size: resultsPerPage,
        });
    },
);
...
export const getRetrieveUrl = createSelector(
    ...
    (url, domain, ids) => {
        if (typeof ids === 'string') {
            return `${url}/${domain}/publication/retrieve/${encodeURIComponent(
                ids,
            )}`;
        }
        return `${url}/${domain}/article/retrieve?dbid=${encodeURIComponent(
            ids.dbId,
        )}&an=${encodeURIComponent(ids.an)}`;
    },
);
...
export const getMetadoreSearchUrl = createSelector(
    [getUrl, getMetadoreQueryString],
    (url, queryString) => `${url}/metadore/search?${queryString}`,
);
...
export const getMetadoreSearchRequest = createSelector(
    [getMetadoreSearchUrl, getToken],
    getRequest,
);
...
export const getApiDeleteHistoriesRequest = createSelector(
    [getUrl, getToken],
    (url, token) => ({
        url: `${url}/histories`,
        config: {
...
export const getApiDisableAlertRequest = createSelector(
    [getUrl, getToken, getProps],
    (url, token, historyId) => {
        return {
            url: `${url}/history/disable/${historyId}`,
            config: {
                credentials: 'include',
                headers: {
                    Authorization: `Bearer ${token}`,
...
export const getApiDisableAllAlertRequest = createSelector(
    [getUrl, getToken],
    (url, token) => {
        return {
            url: `${url}/history/disable`,
            config: {
                credentials: 'include',
                headers: {
                    Authorization: `Bearer ${token}`,
...
export const getCountHistories = createSelector(
    [getUrl, getToken, getProps],
    (url, token) => {
        return {
            url: `${url}/history/count`,
            config: {
                credentials: 'include',
                headers: {
                    Authorization: `Bearer ${token}`,
