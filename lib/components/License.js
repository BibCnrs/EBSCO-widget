...
const License = ({ language, domains }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch(
                `${window.__SERVER_URL__}/licenses?` +
                    new URLSearchParams({
                        domains: domains,
                    }),
                {
    ...
    if (isLoading) {
        <div className="licenses_loading">Loading...</div>;
    }

    if (licenses.length === 0) {
        return <div className="licenses_not_found">No licenses found</div>;
    }
...
export default translate(connect(mapStateToProps)(License));
