...
export const ExactMatchPlacard = ({ publication, notice, noticeShown, text, showNotice, exactMatchSearch }) => {
    if (!publication) {
        return <span/>;
    }
    ...
    return (
        <div className="exact-match">
            ...
            <FormControl
                ...
                onKeyPress={(event) => (event.key === 'Enter' && exactMatchSearch(event.target.value, title, publication.publicationId))}
            />
            { noticeShown && notice ? (
