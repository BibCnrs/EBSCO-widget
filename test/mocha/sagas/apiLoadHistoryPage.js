...
describe('sagas apiLoadHistoryPage', function () {
    ...
    it('should select getApiLoadHistoryRequest', function () {
        assert.deepEqual(iterator.next().value, select(fromState.getApiLoadHistoryRequest));
    });

    it('should trigger loadHistoryPagePending', function () {
        iterator.next();
        assert.deepEqual(iterator.next().value, put(actions.loadHistoryPagePending()));
    });
    ...
    it('should call the fetch for the apiLoadHistoryPageRequest', function () {
        ...
        assert.deepEqual(iterator.next().value, call(fetch, { url: 'foo?limit=5&offset=45' }, [], false));
    });
    ...
    it('should trigger loadHistoryPageError if receiving error', function () {
        ...
        assert.deepEqual(next.value, put(actions.loadHistoryPageError('error')));
        assert.isTrue(iterator.next().done);
    });
    ...
    it('should trigger loadHistoryPageSuccess if receiving response with no error', function () {
        ...
        assert.deepEqual(next.value, put(actions.loadHistoryPageSuccess('response', 10)));
    });
