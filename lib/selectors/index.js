...
export const hasPublicationSearchResult = createSelector(
    [getSearchResult],
    (searchResult) => searchResult.publication && searchResult.publication.totalHits > 0
);
export const {
    getCurrentPage,
    getCurrentPageData,
...
export const getAllDomains = createSelector(
    [getDomains, getProps],
    (domains, category) => category === 'article' ? domains.available : _.uniq([...domains.available, ...domains.all])
);
...
export const getExactMatchRetrieveUrl = createSelector(
    ...
    (url, domain, id) =>
        `${url}/${domain}/publication/retrieve/${id}`
);
...
export const getUpdateProfileData = state => ({ favorite_domain: state.profile.favorite_domain });
...
export const isInA2zMode = createSelector(
    [hasActiveFacet, isDefaultLimiter, isQueryListInA2zMode, isSearchInA2zMode],
    (activeFacet, defaultLimiter, queryListInA2zMode, searchInA2zMode) => !activeFacet && defaultLimiter && queryListInA2zMode && searchInA2zMode
);
...
export const hasAccessToFavoriteDomain = createSelector(
    ...
    (domains = [], { favorite_domain }) => domains && favorite_domain && domains.includes(favorite_domain)
);
...
export const getDomainToChangeOnLogin = createSelector(
    [hasPublicationSearchResult, getLocation],
    (publicationSearchResult) => (publicationSearchResult ? ['article', 'database'] : ['article', 'publication', 'database'])
);
