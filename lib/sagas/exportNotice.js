...
const formatMap = {
    'Exporter en format RIS': 'ris',
    'Exporter en format BibTex (s3448258)': 'bib'
};
...
export const openExport = (notices, format) => {
    ...
    fakeLink.setAttribute('download', `notices.${formatMap[format]}`);
    fakeLink.click();
    fakeLink.remove();
};
...
export function* exportNotice(action) {
    ...
    const exportLinksRequest = yield select(fromState.getExportRequestForIds, { format, ids });
    const { response, error, cancel } = yield call(fetch, exportLinksRequest, [SEARCH, LOGOUT], false);
    ...
    yield call(openExport, response.join(''), format);
