...
const ArticleRecord = React.createClass({
    render: function () {
        const { record, index, isSelected, noticeShown, text, showNotice, selectRecord } = this.props;
        const { id, doi, title, publicationType, authors, source, notice } = record;
        const noticeButton = (
            <BibButton
            ...
            <div className="record" ref="record">
                <h4>
                    <Input
                        ...
                        value={isSelected}
                        checked={isSelected}
                        onClick={() => selectRecord('article', id)}
                    />
                    <a
