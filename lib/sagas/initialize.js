...
export function* initializeDomain(domain) {
...
export function* initializeLogin(domain) {
    ...
    if(response) {
        ...
        if(pausedAction.category === 'article') {
            const isDomainAvailable = yield select(fromState.isDomainAvailable, domain);
            if (isDomainAvailable) {
                yield call(initializeDomain, domain);
            }
        }
...
export function* initialize({ domain }) {
    ...
    if(!isUserLogged) {
        yield call(initializeLogin, domain);
    }

    const isNowLogged = yield select(fromState.isUserLogged);

    if (isUserLogged === isNowLogged && domain) {
        yield call(initializeDomain, domain);
