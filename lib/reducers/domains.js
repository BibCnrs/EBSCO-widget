...
export default function domains(state = defaultState, action) {
    ...
    case INITIALIZE: {
        const defaultDomain = state.defaultDomain;
        if (!defaultDomain) {
            return state;
        }
        const articleDomain = state.available.indexOf(defaultDomain) === -1 ? state.article : defaultDomain;
        const publicationDomain = state.all.indexOf(defaultDomain) === -1 ? state.publication : defaultDomain;
        const a2zDomain = state.all.indexOf(defaultDomain) === -1 ? state.a2z : defaultDomain;
