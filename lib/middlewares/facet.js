...
export const facet = function facet(store, next, action) {
    ...
    switch(action.type) {
    case APPLY_FACET:
        const newFacetValues = state.facets[action.name].newValues.map((v) => v.value);
        const currentFacetValues = state.facets[action.name].values.map((v) => v.value);
        ...
        if(newFacetValues.length > currentFacetValues.length) {
            ...
            `addfacetfilter(${state.facets[action.name].filterId},${newFacets})`
            :
        ...
        if(newFacetValues.length < currentFacetValues.length) {
            ...
            const ebscoAction = `removefacetfiltervalue(${state.facets[action.name].filterId},${deletedFacets})`;
