...
const isOpenAccess = (href, hrefWithIcon) => {
    return href && (hrefWithIcon.includes(href) || HAL_REGEX.test(href.url));
};
...
const ArticleRecordTitleLink = ({
    ...
    if (!isOpenAccess(href, hrefWithIcon) && !isLogged) {
        return (
            <b>
                <TooltipSpan
        ...
        <OALink url={href.url} doi={doi} domain={domain} name={href.name}>
            {({ url }) => (
                <React.Fragment>
                    ...
                    {isOpenAccess(href, hrefWithIcon) && (
                        <TooltipSpan
                            tooltipPlacement="bottom"
                            tooltip={text.openAccessArticleDescription}
