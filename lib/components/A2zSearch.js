...
const A2zSearch = ({ firstLetter, secondLetter, changeLetters, search, DomainSelector }) => {
                ...
                <DomainSelector/>
                {
                    aToZ.map((letter, index) => (
                        <BibButton
                            ...
                            key={index}
                            label={letter}
                            onClick={() => changeLetters(letter, '') || search()}
                        />
                    ))
                }
            </div>
                            ...
                            aToZ.map((letter, index) => {
                                ...
                                return (
                                    <BibButton
                                        ...
                                        key={index}
                                        label={label}
                                        onClick={() => changeLetters(firstLetter, letter) || search()}
                                    />
                                );
                            })
                        }
