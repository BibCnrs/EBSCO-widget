...
const parseAutoComplete = (response) => JSON.parse(response).terms.map(({term}) => term);

export function* loadPage(action) {
    yield call(delay, 500);
    const { response, error, cancel } = yield call(fetch, {
        url: `https://widgets.ebscohost.com/prod/simplekey/autocomplete/autocomp.php?q=${action.term}`
    }, [CHANGE_TERM]);
    if(error || cancel) {
