...
export const isDomainAvailable = (state, location) => state.available.includes(state[location]);
...
export const getDefaultDomain = (state, available = true) => {
    ...
    return domains.includes(state.defaultDomain) ? state.defaultDomain : undefined;
};

export const getDomainValue = (state, location) => {
    if(location === 'article') {
        return state.available.includes(state.defaultDomain) ? state.defaultDomain : state.available[0];
    }

    return state.all.includes(state.defaultDomain) ? state.defaultDomain : state.all[0];
};
...
export const getDomainChange = (state) => {
    ...
    return {
        article: state.article !== defaultAvailableDomain ? defaultAvailableDomain || state.available[0] : undefined,
        publication: state.publication !== defaultDomain ? defaultDomain : undefined,
