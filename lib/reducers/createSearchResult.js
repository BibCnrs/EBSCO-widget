...
export const getRecordIdsWithMissingLink = (state) => {
    return state[state.currentPage].filter(id => {
        ...
        const nbFullTextLinks = articleLinks.fullTextLinks ? articleLinks.fullTextLinks.length : 0;
        const nbPdfLinks = articleLinks.pdfLinks ? articleLinks.pdfLinks.length : 0;
