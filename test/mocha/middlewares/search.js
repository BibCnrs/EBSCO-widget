    ...
    const testType = (type) => {
        ...
        assert.deepEqual(dispatchedAction, [
            actions.article.search(
                `${state.url}/${state.article.search.domain}/article/search?queries=${encodeURIComponent(JSON.stringify([{ term: state.article.search.term }]))}&FT=Y&DT1=${from}-01/${to}-01&currentPage=5`,
                state.login.token,
                {
                    term: 'searched term',
                    domain: 'vie',
