...
export const getDefaultDomainFromList = (state, list) => {
    if (state.setFromUrl && list.indexOf(state.setFromUrl) !== -1) {
        return state.setFromUrl;
    }
    if (state.favoriteDomain && list.indexOf(state.favoriteDomain) !== -1) {
        return state.favoriteDomain;
    }

    return list.indexOf(state.defaultDomain) !== -1 ? state.defaultDomain : undefined;
};

export const getDefaultDomain = (state, location) => {
    if (location === 'article') {
        return getDefaultDomainFromList(state, state.available);
    }

    return getDefaultDomainFromList(state, state.all);
};
