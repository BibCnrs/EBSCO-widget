...
export default class Limiters extends Component {
    render() {
        const { publicationDate, onLimitSearch, onChangeLimiter } = this.props;
            ...
            <div className="publication-date-limiter">
                ...
                <div>
                    ...
                    <DatePicker
                        ...
                        date={from}
                        onChange={ (date) => (onChangeLimiter('publicationDate', { from: date, to }), onLimitSearch())}
                        minView={1}
                    />
                </div>
                <div>
                    ...
                    <DatePicker
                        ...
                        computableFormat="YYYY-MM"
                        date={to}
                        onChange={ (date) => (onChangeLimiter({ from, to: date }), onLimitSearch())}
                        minView={1}
                    />
                </div>
                <div className="slider">
                    <RcSlider
                        ...
                        step={1}
                        range={true}
                        onChange={ (values) => onChangeLimiter('publicationDate', {
                            from: `${values[0]}-${from.substr(-2)}`,
                            to: `${values[1]}-${to.substr(-2)}`
                        })}
