...
const ArticleRecord = React.createClass({
    render: function () {
        const { record, isSelected, noticeShown, notice, exporting, text, showNotice, selectRecord, exportNotice } = this.props;
        ...
        return (
            <div className="record" ref="record">
                ...
                {noticeButton}
                <ArticleLink id={id} />
                <BibButton label={text.export} icon={exporting ? { name: 'spinner', spin: true } : { name: 'download' }} onClick={() => exportNotice([id])}/>
                <p>{publicationType}</p>
