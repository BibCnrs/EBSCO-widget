...
import { LOADING, LOADED, ARTICLE, PUBLICATION, A2Z} from '../actions';
...
export function fetchMiddleware(next, action) {
    ...
    if(
        action.type !== ARTICLE.RETRIEVE_LINK
        && action.type !== ARTICLE.RETRIEVE
        && action.type !== PUBLICATION.RETRIEVE
        && action.type !== A2Z.RETRIEVE
    ) {
        next({
            ...action,
            type: LOADING
