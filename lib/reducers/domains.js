...
export const getDefaultDomain = (state, available = true) => {
    ...
    if (state.setFromUrl && domains.indexOf(state.setFromUrl) !== -1) {
        return state.setFromUrl;
    }
    if (state.favoriteDomain && domains.indexOf(state.favoriteDomain) !== -1) {
        return state.favoriteDomain;
    }

    return domains.indexOf(state.defaultDomain) !== -1 ? state.defaultDomain : undefined;
};
