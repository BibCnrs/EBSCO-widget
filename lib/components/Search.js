...
import { Input, DropdownButton, MenuItem, Grid, Row, Col } from 'react-bootstrap';
...
class Search extends Component {
    ...
    render() {
        ...
        const select = (
            ...
            >
                {domains.map((domain, index) => (<MenuItem onClick={() => onChangeDomain(domain)} key={index} value={domain}>{domain}</MenuItem>))}
            </DropdownButton>
        );
