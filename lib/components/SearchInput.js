...
const SearchInput = ({ label, placeholder, value, suggestedValues, buttonAfter, buttonBefore, onChange, onApply }) => {
    return (
                ...
                <InputGroup>
                    {suggestedValues.map((val, index) => (
                        <InputGroup.Button key={index}>
                            <Button bsStyle="link" onClick={() => onChange(`${value}${val}`)}><span><strong>{value}</strong>{val}</span></Button>
                        </InputGroup.Button>))}
                </InputGroup>
            </InputGroup>
        </div>
