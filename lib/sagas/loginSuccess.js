...
export function* loginSuccess({ response }) {
    const { username, domains, favorite_domain, origin } = response;
    ...
    const hasDomainSetFromUrl = yield select(fromState.hasDomainSetFromUrl);
    if(!hasPublicationSearchResult && !hasDomainSetFromUrl) {
        if(favorite_domain) {
